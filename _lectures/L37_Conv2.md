<!-- ---
type: lecture
date: 2025-10-17T12:15:00
title: "Lecture 37: Padding and Multichannel Convolution"
tldr: "Convolution II"
stat: lec
# for lectures stat: lec
description: We further learn about zero padding, which helps us flexibly play with the output dimensions. We then extend the basic 2D convolution to the multi-channel case, where a multi-channel input is processed to a multi-channel output. 
videoID: d_AWF37aUnY 
hide_from_announcments: false
---
**Lecture Notes:**
- [Chapter 4 - Section 2]({{ site.baseurl }}/assets/Notes/CH4/CH4_Sec2.pdf) 

**Further Reads:**
* [Multi-channel Convolution](https://www.bishopbook.com/): Chapter 10 - Sections 10.2.3 to 10.2.5 of [[BB]](https://www.bishopbook.com/) -->
