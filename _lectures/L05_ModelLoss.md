<!-- ---
type: lecture
date: 2025-09-05T11:40:00
title: "Lecture 5: Components 2 and 3: Model and Loss"
tldr: "ML Part III - Model and Loss"
stat: lec
# for lectures stat: lec
description: We now define formally the notions of a computational model and a loss function. In this respect, we understand what hyperparameters and learnable parameters are.
videoID: eQ09NRY3awU 
hide_from_announcments: false
---
**Lecture Notes:**
- [Chapter 1 - Section 2]({{ site.baseurl }}/assets/Notes/CH1/CH1_Sec2.pdf) 

**Further Reads:**
* [ML Components](https://www.bishopbook.com/): Chapter 1 - Sections 1.2.1 to 1.2.4 of [[BB]](https://www.bishopbook.com/)
* [ML Basics](https://www.deeplearningbook.org/): Chapter 5 of [[GYC]](https://www.deeplearningbook.org/) -->