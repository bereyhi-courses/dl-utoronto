<!-- ---
type: lecture
date: 2025-10-07T11:10:00
title: "Lecture 28: Sources of Overfitting"
tldr: "Overfitting"
stat: lec
# for lectures stat: lec
description: We talk about three key concepts, namely model complexity, data size, and co-adaptation. These factors all contribute to overfitting. We talk about solutions to handle each of these situations. 
videoID: gH4mluttEdg 
hide_from_announcments: false
---
**Lecture Notes:**
- [Chapter 3 - Section 2]({{ site.baseurl }}/assets/Notes/CH3/CH3_Sec2.pdf) 

**Further Reads:**
* [Overfitting and Regularization](https://www.bishopbook.com/): Chapter 9 - Sections 9.1 to 9.3 of [[BB]](https://www.bishopbook.com/) -->
